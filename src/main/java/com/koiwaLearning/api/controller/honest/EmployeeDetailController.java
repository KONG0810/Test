...
public class EmployeeDetailController {
     ...
     * 跳转
     * 
     * /1/edit => @PathVariable(value = "eid") 
     * ?eID=1 => @RequestParam(value ="eId")
     */
    ...
    public ModelAndView employeeEdit(Model model, @PathVariable(value = "eid", required = true) Integer eid) {
        ...
        model.addAttribute("name", eDetail.getEmployeeName());
        model.addAttribute("info", eDetail);
        return new ModelAndView("honest/employeeDetail", "employeeDetail", model);
    }

    @RequestMapping(value = { "/{eid:\\d+}/search" }, method = RequestMethod.GET)
    public Employee employeeSearch(Model model, @PathVariable(value = "eid", required = true) Integer eid) {
        // System.out.println(eid);
        Employee eDetail = EmployeeService.getEmployeeDetail(eid);
        model.addAttribute("name", eDetail.getEmployeeName());
        model.addAttribute("info", eDetail);
    ...
    public void deleteEmployee(@RequestBody EmployeeId eId) {
        System.out.println("delete");
        System.out.println(eId.getEId());
        Employee eDetail = EmployeeService.getEmployeeDetail(eId.getEId());
    ...
    public void addEmployee(@RequestBody Employee user) {
        ...
        return;
        // return new ModelAndView("honest/employeeDetail");
    }
